      ...
      .update(
        Object.assign({}, ...request, {
          avatar_image: req.file
            ? `${process.env.SERVER_URL}:${process.env.SERVER_PORT}/avatarImages/${req.file.filename}`
            : undefined,
        })
      );
...
module.exports.addUserBook = async (req, res) => {
  try {
    ...
    await db("book").insert({
      ...
      cover_image: `${
        req.file
          ? `${process.env.SERVER_URL}:${process.env.SERVER_PORT}/bookCovers/${req.file.filename}`
          : cover_image
      }`,
